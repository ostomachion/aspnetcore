...
""";
    ...
    {
        BodyStruct expectedResult = default;
        var source = $"""
app.MapPost("/", ([CustomFromBody(AllowEmpty = true)] BodyStruct bodyStruct, HttpContext httpContext) => httpContext.Items["value"] = bodyStruct);
""";
        var (_, compilation) = await RunGeneratorAsync(source);
        var endpoint = GetEndpointFromCompilation(compilation);
        ...
        Assert.Equal(expectedResult, httpContext.Items["value"]);
    }
}
